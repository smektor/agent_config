...
After a planning or research session, export all resulting tasks as a single JSON file to `~/tasks/<repo_name>/`. The file is consumed by a Claude agent that implements the tasks. Tasks must be as small as possible — one concern per task.
